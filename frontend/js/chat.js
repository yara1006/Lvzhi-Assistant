...
function appendBubble(m) {
  ...
  div.innerHTML = `<div class="msg-meta">${m.role === 'user' ? '您' : '律智助手'} · ${t}</div><div class="msg-bubble">${esc(m.content)}</div>`;
  el.appendChild(div);
  scrollBot();
}
...
async function renderHistoryFromServer(forceRefresh = false) {
  ...
  try {
    ...
    el.innerHTML = sorted.map(s => {
      ...
      return `
        <div class="history-item ${activeChatId === s.id ? 'active' : ''}" onclick="onHistoryItemClick(event, '${esc(s.id)}', '${esc(s.title)}')">
          <div class="history-icon">${icon}</div>
          <div class="history-content">
            <div class="hi-text">${esc(s.title)}</div>
            <div class="hi-tool">${toolName}</div>
          </div>
          <div class="hi-del" onclick="deleteSessionFromList('${esc(s.id)}', event); return false;">✕</div>
        </div>
      `;
    }).join('');
  } finally {
